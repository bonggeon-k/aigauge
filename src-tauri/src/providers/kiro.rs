...
static PLAN_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?im)\b(KIRO[\w\s\-_]+?)\b(?:\||\n|$)").expect("valid plan regex"));
static CREDITS_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\(([\d,]+(?:\.\d+)?)\s+of\s+([\d,]+(?:\.\d+)?)").expect("valid credits regex")
});
static CREDITS_SLASH_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"([\d,]+(?:\.\d+)?)\s*/\s*([\d,]+(?:\.\d+)?)").expect("valid credits slash regex")
});
static PERCENT_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(\d{1,3})\s*%").expect("valid percent regex"));
static RESET_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"resets on\s+(\d{2}/\d{2}(?:/\d{4})?|\d{4}-\d{2}-\d{2})")
...
impl KiroProvider {
    ...
    fn usage_command_candidates() -> Vec<(String, Vec<String>)> {
        ...
        if platform::detect_host_platform() == platform::HostPlatform::Windows
            && platform::has_wsl()
        {
            commands.extend([
                (
                ...
                (
                    ...
                    vec![
                        ...
                        "bash".to_string(),
                        "-lc".to_string(),
                        "kiro chat --no-interactive /usage".to_string(),
                    ],
                ),
            ]);
        }
    ...
    async fn run_usage_command() -> std::result::Result<String, String> {
        ...
        for (program, args) in Self::usage_command_candidates() {
            ...
            let output = match output {
                ...
                Ok(Ok(Err(error))) => {
                    errors.push(format!("{command_debug} -> {error}"));
                    continue;
                }
                Ok(Err(error)) => {
                    errors.push(format!("{command_debug} -> task failed: {error}"));
                    continue;
                }
                Err(_) => {
                    errors.push(format!("{command_debug} -> timed out"));
                    continue;
                }
            };
            ...
            if !output.status.success() {
                ...
                errors.push(format!("{} -> {}", command_debug, stderr.trim()));
                continue;
            }
            ...
            if combined.trim().is_empty() {
                errors.push(format!("{command_debug} -> returned no output"));
                continue;
            }

            if Self::parse_output(combined.as_str()).is_none() {
                errors.push(format!("{command_debug} -> unrecognized usage output"));
                continue;
            }
    ...
    fn parse_output(raw: &str) -> Option<ParsedKiroUsage> {
        ...
        let parse_credit = |index: usize| -> f64 {
            CREDITS_RE
                .captures(target)
                ...
                .or_else(|| {
                    // Fallback: extract the first two numeric values when structured labels are missing.
                    let mut numbers = target.split_whitespace().filter_map(|chunk| {
                        let cleaned = chunk
        ...
        let mut used_credits = parse_credit(1);
        let mut limit_credits = parse_credit(2);
        ...
        let percent_used = PERCENT_RE
            ...
            .unwrap_or_else(|| {
                if limit_credits > 0.0 {
                    ((used_credits / limit_credits) * 100.0)
                        .round()
        ...
        if limit_credits <= 0.0 && percent_used > 0 {
            // Keep progress meaningful when CLI output only includes percentages.
            limit_credits = 100.0;
            used_credits = percent_used as f64;
        }
    ...
    fn classify_command_error(error: &str) -> ParsedUsageResult {
        ...
        if lowered.contains("command not found")
            ...
            || lowered.contains("please login")
            || lowered.contains("please sign in")
        {
...
impl Provider for KiroProvider {
    ...
    async fn provider_info(&self) -> ProviderInfo {
        ...
        ProviderInfo {
            ...
            supported_auth_modes: vec![AuthSourceMode::Cli],
            default_auth_mode: AuthSourceMode::Cli,
            plan_name: plan,
            quota_limit: 100,
...
"#;
    ...
    #[test]
    fn parse_output_without_estimated_usage_header() {
        let sample = "KIRO TEAM\nCredits (12 of 120)\n10%\nresets on 2026-03-15\n";
        let parsed =
            KiroProvider::parse_output(sample).expect("should parse output without header");
        assert_eq!(parsed.plan, "KIRO TEAM");
        assert_eq!(parsed.percent_used, 10);
        assert_eq!(parsed.limit_credits.round() as u64, 120);
        assert_eq!(parsed.reset_at, "2026-03-15");
    }
}
